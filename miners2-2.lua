...
--		end
	...
	function loadRocks()
		...
		bg:toBack()
		transition.to(rockTable[numRocks],{time = math.random(4000,7000), y = math.random(h+100,h*1.25),
...
--	else
	...
	bg:toBack()
	transition.to(gemTable[numGems],{time = math.random(6000,8000), y = math.random(h+100,h*1.25)})
	end
	...
	function gameLoop()
		...
		if(score-score_2>=500 and gemLoop~=nil)then
			timer.pause(gemLoop)
	    	timer.pause(rockLoop)
	    	timer.pause(heartLoop)
	    	timer.pause(shields)
