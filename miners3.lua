...
function scene:create ( event )
	...
	beginButton.alpha=0
	local scoreGoal = display.newImage(gameGroup,"numbers/5000.png")
	scoreGoal.x = w/2
	scoreGoal.y = goal.y
	scoreGoal.alpha=0
	...
	--The shield
	local shield = display.newImage(gameGroup,"redShield.png")
	...
	local cavernNum=display.newImage(pauseGroup, "numbers/3.png")
	...
	function startGame()
	    ...
	    rockLoop = timer.performWithDelay(1400, loadRocks,0)
	...
	function loadRocks()
		...
		bg:toBack()
		transition.to(rockTable[numRocks],{time = math.random(3800,5500), y = math.random(fallingObjectDestinationPoint,fallingObjectDestinationPoint+200),
	...
	function loadGem()
		...
		bg:toBack()
		transition.to(gemTable[numGems],{time = math.random(5000,6500), y = math.random(fallingObjectDestinationPoint,fallingObjectDestinationPoint+200)})
	end
	...
	function loadShields()
		if(shieldActive==false) then
			...
			if (num<=70) then
				...
				shieldTable[numShields]=display.newImage(gameGroup,"red_shield.png")
...
--		lose.alpha = 0
	...
	function onward()
		...
		textLives:removeSelf()
   	 textScore:removeSelf()
		composer.gotoScene("miners4","crossFade",500)
	end
