...
--		end
	...
	function cleanup(action)
		...
		elseif (action == "upkeep")then
			...
			for i=1,table.getn(shieldTable) do
				if(shieldTable[i].myName~= nil and shieldTable[i].y > h+100) then
					shieldTable[i]:removeSelf()
					shieldTable[i].myName=nil
				end
			end
	...
	function loadRocks()
		...
		bg:toBack()
		transition.to(rockTable[numRocks],{time = math.random(3000,5000), y = math.random(h+100,h*1.25),
...
--	else
	...
	function gameLoop()
		...
		if(score-score_3>=1000 and gemLoop~=nil)then
			timer.pause(gemLoop)
	    	timer.pause(rockLoop)
	    	timer.pause(heartLoop)
	    	timer.pause(shields)
