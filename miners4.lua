...
function scene:create ( event )
	...
	beginButton.alpha=0
	local scoreGoal = display.newImage(gameGroup,"numbers/10000.png")
	scoreGoal.x = w/2
	scoreGoal.y = goal.y
	scoreGoal.alpha=0
	...
	local cavernNum=display.newImage(pauseGroup, "numbers/4.png")
	...
	local explosion = audio.loadSound("sounds/explosion.mp3")
	...
	function startGame()
	    ...
	    rockLoop = timer.performWithDelay(1000, loadRocks,0)
	...
	function loadRocks()
		...
		bg:toBack()
		transition.to(rockTable[numRocks],{time = math.random(3500,5000), y = math.random(fallingObjectDestinationPoint,fallingObjectDestinationPoint+200),
	...
	function loadGem()
		...
		bg:toBack()
		transition.to(gemTable[numGems],{time = math.random(5000,6000), y = math.random(fallingObjectDestinationPoint,fallingObjectDestinationPoint+200)})
	end
	...
	function loadHeart()
		if(lives<5)then
			...
			heartTable[numHearts] =  display.newImage(gameGroup,"heart2.png")
				...
				bg:toBack()
				transition.to(heartTable[numHearts],{time = 6000, y = math.random(fallingObjectDestinationPoint,fallingObjectDestinationPoint+200)})
			end
		end
	end
	...
	function loadShields()
		if(shieldActive==false) then
			...
			if (num<=70) then
				...
				shieldTable[numShields].myName = "purple_shield"
...
--		lose.alpha = 0
	...
	function onCollision(event)
		...
		if(event.object1.myName =="mine_cart" and event.object2.myName == "purple_shield") or (event.object1.myName =="purple_shield" and event.object2.myName == "mine_cart") then

			if (event.object1.myName == "purple_shield") then
				event.object1:removeSelf()
				event.object1.myName=nil
			else
				event.object2.myName=nil
